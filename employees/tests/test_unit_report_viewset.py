...
from employees.views import AuthorReportList
...
class AuthorReportListTests(TestCase):
    def setUp(self):
        self.user = CustomUser(
            email="testuser@codepoets.it", password="newuserpasswd", first_name="John", last_name="Doe", country="PL"
        )
    ...
    def test_author_report_list_view_should_display_users_report_list_on_get(self):
        request = APIRequestFactory().get(path=reverse("author-report-list", args=(self.user.pk,)))
        request.user = self.user
        response = AuthorReportList.as_view()(request, pk=self.user.pk)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.report.description)
        dictionary = response.data["reports_dict"]
        reports = list(dictionary.values())[0]
    ...
    def test_author_report_list_view_should_not_be_accessible_for_unauthenticated_user(self):
        request = APIRequestFactory().get(path=reverse("author-report-list", args=(self.user.pk,)))
        request.user = AnonymousUser()
        response = AuthorReportList.as_view()(request, pk=self.user.pk)
        self.assertEqual(response.status_code, 403)

    def test_author_report_list_view_should_not_display_non_existing_users_reports(self):
        request = APIRequestFactory().get(path=reverse("author-report-list", args=(999,)))
        request.user = self.user
        response = AuthorReportList.as_view()(request, 999)
        self.assertEqual(response.status_code, 404)
    ...
    def test_author_report_list_view_should_display_message_if_user_has_no_reports(self):
        ...
        other_user.full_clean()
        other_user.save()
        request = APIRequestFactory().get(path=reverse("author-report-list", args=(other_user.pk,)))
        request.user = self.user
        response = AuthorReportList.as_view()(request, pk=other_user.pk)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, AuthorReportListStrings.NO_REPORTS_MESSAGE.value)

    def test_author_report_list_get_queryset_method_should_return_queryset_containing_all_of_users_reports(self):
        other_user = CustomUser(
            email="otheruser@codepoets.it", password="otheruserpasswd", first_name="Jane", last_name="Doe", country="PL"
        )
        other_user.full_clean()
        ...
        request = APIRequestFactory().get(path=reverse("author-report-list", args=(self.user.pk,)))
        request.user = self.user
        view = AuthorReportList()
        view.request = request
        queryset = view.get_queryset(pk=self.user.pk)
